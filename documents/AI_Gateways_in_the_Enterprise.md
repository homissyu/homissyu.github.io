...
## 인공지능(AI) 및 대규모 언어 모델(LLM) 구현의 과제
...
여기서 AI 게이트웨이가 나옵니다. 이들은 애플리케이션과 AI 서비스 사이에 위치하여 이러한 과제들에 대한 포괄적인 솔루션을 제공하는 특화된 도구입니다. AI 게이트웨이는 AI 운영을 관리하기 위한 제어 평면 역할을 하며, 조직 전체의 의미론적 요청 라우팅, 지능형 캐싱, 비용 최적화, 그리고 AI 제공자 페일오버 기능을 제공합니다. 이러한 문제들을 애플리케이션 수준이 아닌 인프라 수준에서 처리함으로써, AI 게이트웨이는 조직이 모든 AI 워크로드에 걸쳐 일관된 정책과 최적화를 구현할 수 있게 하면서 애플리케이션 개발자의 복잡성을 줄입니다.

## 끊김 없는 AI통합의 장애물

조직들이 AI의 변혁적 잠재력을 이해하고 있음에도 불구하고, 기존 엔터프라이즈 환경으로의 성공적인 통합을 방해할 수 있는 몇 가지 근본적인 장애물이 있습니다.
...
### 기존 보안 관행
가장 먼저 눈에 띄는 것은 LLM에 대한 호출이 헤더 중 하나로 전송되는 API 키를 필요로 한다는 점입니다. 이 API 키는 민감한 정보이며 전송 중에 암호화되도록 HTTPS 요청에서만 전송되어야 합니다. OpenAI API를 호출할 때(즉, https://api.openai.com을 호출함으로써) 이는 투명하게 발생합니다. 하지만 요청에 사용될 키를 저장하고 검색하는 것은 어떻게 해야 할까요? 개발자들은 어떻게 키를 안전하게 유지할까요? 키가 악의적인 행위이든 다른 팀과 공유함으로써이든 손상되면 보안 문제가 됩니다.
...
관찰성은 LLM 상호작용을 관리하는 또 다른 중요한 측면입니다. 엔터프라이즈는 LLM 워크플로우에서 성능 병목 현상이나 시스템 문제를 식별하기 위해 오류율 및 지연시간과 같은 주요 네트워킹 메트릭을 모니터링해야 합니다. 그러나 LLM은 전통적인 관찰성 관행을 넘어서는 추가적이고 모델 특화된 우려사항들을 도입합니다. 예를 들어, 프롬프트 진화 추적(프롬프트가 과도하게 길지 않은가? 구조가 최적이 아닌 결과를 제공하도록 변경되고 있지 않은가?), 토큰 지연시간(TTFT, 출력 토큰당 시간 등), 문맥 윈도우 사용량, 그리고 사용량 할당은 모두 우려사항입니다.
...
## 안전하고 확장 가능한 AI 도입에서 AI 게이트웨이의 역할
...
Figure 2-2. 포워딩 프록시 모델을 사용한 LLM  요청
...
AI 게이트웨이는 요청/프롬프트의 의미론적 이해를 활용하기 때문에 로드 밸런싱, 페일오버 또는 결과 캐싱에 대해 더 스마트한 결정을 내릴 수 있습니다. 의미론적 인식은 게이트웨이가 조직에서 승인한 방법에 따라 프롬프트에 추가 문맥을 첨부할 수 있게 해줍니다. 예를 들어, AI 게이트웨이는 검색 증강 생성(RAG)을 구현하는 데 도움이 될 수 있으며, 이는 LLM을 더 정확한 응답으로 안내하기 위해 추가 사실이나 조직별 데이터를 프롬프트에 추가하는 메커니즘입니다.

## AI 게이트웨이 설계 및 구현 최적 사례
많은 조직들이 내부 개발자 플랫폼을 중심으로 개발자를 위한 고도로 자동화된 자가 서비스 워크플로우로 이동하고 있으며, AI 게이트웨이는 이 플랫폼과 호환되어야 합니다. 우리는 이 장의 마지막 섹션에서 이것에 대해 더 다룰 것이지만, 이 통합을 용이하게 하려면 AI 게이트웨이 설계를 고려할 때 다음을 고려해야 합니다:
...
### 오픈소스 기반을 고려하라
AI 게이트웨이를 오픈 소스 기반 위에 구축하면 몇 가지 이점을 얻을 수 있습니다. Envoy와 같은 오픈 소스 기술은 지속적인 혁신, 보안 업데이트, 기능 개발에 기여하는 크고 활발한 커뮤니티와 함께 제공됩니다. 이는 벤더 종속성을 줄이고 엔터프라이즈가 발전하는 기술 요구사항을 앞서나갈 수 있도록 해줍니다. 오픈 소스 소프트웨어를 활용하는 것은 현대의 개발 관행과 부합하며, 특정 엔터프라이즈 요구사항을 충족하도록 게이트웨이를 조정할 수 있는 비용 효율성과 유연성을 제공합니다.

## AI 성공을 위한 플랫폼 엔지니어링의 필요성
IT 조직은 종종 여러 팀을 같은 페이지에 놓기 위해 고민합니다. 팀들은 종종 사일로에서 결정을 내리다 보니 도구의 중복, 통합 도전, 거버넌스 붕괴로 이어지며, 이는 규정 준수 문제를 야기할 수 있습니다. 플랫폼 엔지니어링은 다양한 IT 팀을 통합하여 소프트웨어 전달과 운영 우수성을 간소화하는 데 중요한 역할을 합니다. 애플리케이션 개발자, 보안 전문가, 네트워킹 전문가 등 다양한 IT 팀 간의 협업을 촉진함으로써, 플랫폼 엔지니어링 계획은 마찰을 줄이고, 개발자 생산성을 향상시키며, 규정 준수 표준을 강제하는 것을 목표로 합니다.
...
Figure 3-3. 
RBAC을 사용하여 AI Gateway를 통하는 AI연구팀은 허용, QA팀은 차단하는 아키텍처
...
Figure 3-4. JWT의 주체별 할당량 기반 토큰 제한

### AI 워크플로우에서 다중 테넌시 보안
...
Figure 3-5. AI gateway를 통한 입출력 가드레일 기반 콘텐츠 무해화 워크플로우
...
Figure 3-6. 표준화된 LLM 상호작용을 위한 사전정의 템플릿과 사용자 입력
...
**TTFT**는 쿼리 제출부터 첫 번째 생성 토큰까지의 초기 응답 시간을 측정합니다. 이 메트릭은 실시간 애플리케이션(예: 챗봇)에서 중요하며, 사용자는 즉각적인 응답을 기대합니다. AI 게이트웨이의 트래픽 관리 결정은 라우팅 및 부하 분산을 최적화하여 TTFT에 큰 영향을 미칠 수 있습니다.

**TPOT**은 이후 토큰들이 생성되는 속도를 측정합니다. 스트리밍 응답의 경우, TPOT은 인간의 읽기 속도(텍스트 복잡성과 읽기 속도에 따라 대략 초당 5–6 토큰)와 일치해야 부드러운 사용자 경험을 보장합니다. AI 게이트웨이는 지능형 로드밸런싱과 자원 할당을 통해 TPOT을 최적화할 수 있습니다.
...
Figure 3-7. LLM 응답 생성 과정의 지연 요소
...
Figure 3-8. 벡터 데이터베이스를 사용한 의미적으로 유사한 요청 검색

의미론적 캐싱을 구현하려면 쿼리 의도를 분석하기 위해 게이트웨이 수준에서 임베딩 모델이나 소형 LLM이 필요하며, 이는 고유한 계산 오버헤드와 복잡성을 도입한다는 점을 주목하는 것이 중요합니다. 이는 조직이 게이트웨이 비용과 LLM 호출 감소로 인한 잠재적 절감 간의 트레이드오프를 신중하게 평가해야 한다는 의미입니다.

의미론적 캐싱의 비용 절감 효과는 상당할 수 있습니다. 앞의 예를 계속하면, GPT-4를 사용하는 월별 고객 쿼리가 100,000건이고 쿼리당 평균 비용이 $0.03이라고 가정합시다. 의미론적 캐싱이 없으면 월 비용은 $3,000입니다. 의미론적 캐싱이 60%의 쿼리를 처리할 수 있다면(일반적인 고객 서비스 시나리오의 보수적 추정치), 월 비용은 $1,200으로 감소하여 $1,800의 절감이 이루어집니다. 1년에 걸치면 이는 LLM 직접 비용에서 $21,600의 감소를 나타내며, 감소된 계산 자원과 개선된 응답 시간으로부터의 추가 절감은 포함하지 않습니다.

지연시간에 영향을 미치고 인지된 반응성과 TTFT 메트릭을 개선하는 또 다른 최적화는 스트리밍 모드입니다. 스트리밍 모드에서는 LLM이 전체 응답이 완료될 때까지 기다리지 않고 생성되는 토큰을 클라이언트로 전송하여 사용자가 응답이 실시간으로 점진적으로 구축되는 것을 볼 수 있게 합니다. 스트리밍 모드는 LLM 백엔드에서 제공되어야 하며 AI 게이트웨이에서 구현되고 활성화되어야 합니다.

### AI 파이프라인의 모니터링과 가시성

모든 애플리케이션과 마찬가지로 AI와 LLM을 사용하는 애플리케이션을 실행할 때 우수한 모니터링과 관찰성이 중요합니다. 측정 및 비용의 운영 단위는 토큰입니다. 이들은 조직이 청구받는 단위들입니다. 입력 토큰과 출력 토큰 모두 중요합니다: 입력 토큰은 LLM으로 전송되는 프롬프트와 쿼리이며, 출력 토큰은 LLM으로부터의 응답을 나타냅니다. 둘 다의 비용은 일반적으로 백만 토큰당으로 표시되며 LLM 제공자에 따라 다릅니다.

AI 제공자와 사용 중인 모델에 따라, 입력 토큰 백만 개당 몇 센트에서 몇 달러까지 비용이 발생할 것으로 예상하고, 출력 토큰 응답은 일반적으로 여러 배 더 비쌉니다.

토큰의 다양한 비용을 감안하면, 포괄적인 모니터링이 왜 중요한지 명확합니다. 효율적인 관찰성 시스템은 조직이 문제를 조기에 식별하고, 모델 사용을 최적화하며, 비용을 추적하고, 예산을 초과하지 않도록 보장할 수 있게 해줍니다.

"성능 메트릭 및 사용자 경험"에서 우리는 성능 메트릭인 TTFT와 TPOT에 대해 이야기했습니다. 다음은 범주별로 그룹화된 AI 애플리케이션에 귀중한 통찰력을 제공할 수 있는 기타 메트릭들입니다:

- Resource utilization

  - 모델 및 엔드포인트별 토큰 소비 패턴
  - 서로 다른 LLM 제공자 간 요청 분배
  - 의미론적 캐시 적중률 및 효율성 메트릭
  - 동시 요청 패턴 및 큐 깊이

- Quality metrics

  - 완료 성공률
  - 오류율 및 유형(문맥 길이, 콘텐츠 필터링, 타임아웃)
  - 응답 일관성 점수
  - 환각(hallucination) 탐지율
...
AI 게이트웨이는 보안, 성능 최적화, 관찰성 과제에 대한 포괄적인 솔루션을 제공함으로써 엔터프라이즈 AI 도입의 중요한 기반이 됩니다. 중앙 집중식 자격증명 관리, 의미론적 캐싱, 상세한 모니터링 기능 같은 기능들은 조직이 비용, 보안, 규정 준수 요구사항에 대한 통제를 유지하면서 AI 구현을 안전하게 확대할 수 있게 해줍니다.

다음 장에서는 이러한 기초적 기능들이 미래에 어떻게 진화할 것인지 살펴보겠습니다.

# Chapter 4. 기업 AI의 미래
AI는 정보기술에서 흥미롭고 혁신적인 시대를 가져오고 있습니다. 많은 조직들이 AI를 활용하여 비즈니스 워크플로우를 최적화하고 효율화하거나 혁신하여 더 나은 사용자 경험을 제공하고 있지만, 점점 더 많은 사용 사례가 이 기술을 활용할 것이라고 예상됩니다. AI 사용 사례와 도입을 확대하려면 이 보고서에서 논의한 AI 게이트웨이와 같이 "AI 인식형" 특화된 도구가 필요합니다. 엔터프라이즈의 미래 AI가 규모, 보안, 관찰성, 규정 준수의 과제에 어떻게 기여하는지 살펴보겠습니다.

엔터프라이즈가 AI 및 LLM 기반 사용 사례에 계속 혁신하고 투자할 것으로 예상합니다. LLM은 시스템 모니터링, 사고 대응, 인프라 최적화와 같은 반복적 작업 자동화를 돕고 있으며, 이를 통해 더 빠른 해결 시간과 향상된 효율성을 실현합니다. 보안 및 위협 탐지 측면에서는 이러한 모델이 패턴 식별, 이상 탐지, 사이버 위협에 대한 실시간 대응 능력을 강화합니다. 고객 서비스 및 지원도 변화를 겪고 있으며, AI 기반 챗봇과 가상 어시스턴트가 24시간 맞춤형 지원을 제공하여 고객 만족도를 높이고 운영 비용을 줄입니다. 이러한 투자들은 전략적 비즈니스 성과를 주도하고 기업이 경쟁 우위를 유지하는 데 도움이 됩니다.

## 모델 개선, 지속적 채택

AI 기반 사용 사례의 지속적 확장과 함께, 기본 모델이 더욱 강력해지고 저렴해질 것으로 예상합니다. 향후 1년 동안 LLM은 여러 주요 영역에서 상당한 진전을 이룰 것으로 예상합니다. 문맥 윈도우(context window)는 드라마틱하게 확장되고 있으며, Anthropic의 Claude 같은 일부 모델은 이미 100,000 토큰에 도달했고, Google의 Gemini는 최대 200만 토큰을 지원합니다. 이 수치들은 당신이 이 글을 읽을 때쯤에는 구식이 될 수 있지만, 이러한 모델들이 계속해서 문맥 능력을 확장하여 단일 상호작용에서 훨씬 더 많은 콘텐츠와 데이터셋을 분석할 수 있게 될 것이라는 사실은 변하지 않을 것입니다. 추론 능력(reasoning capabilities)도 빠르게 개선되고 있으며, OpenAI의 o1 및 o3 모델에서 볼 수 있듯이 강화된 논리적 일관성, 다단계 문제 해결, 상황 이해를 강조하고 있습니다. 마지막으로, 최근 DeepSeek-R1 출시 주장에서 볼 수 있듯이 이러한 모델을 훈련하는 비용이 감소할 가능성이 높습니다. 이러한 진행은 AI가 엔터프라이즈 도메인 전반에 걸쳐 복잡하고 미묘한 쿼리를 다루는 데 더욱 능숙해지도록 할 것입니다.

## 내부모델 채택

엔터프라이즈는 소형 언어 모델(SLM) 또는 Llama, Mistral 및 기타와 같은 오픈 소스 옵션을 활용하여 자체 언어 모델을 실행하고 훈련할 수 있는 가능성을 점점 더 탐색하고 있습니다. 이 추세는 데이터 프라이버시, 비용 효율성, 맞춤화에 대한 더 큰 통제 필요성으로부터 촉발되었습니다. SLM 및 오픈 소스 모델은 조직이 제3자 공급업체에 대한 의존도를 줄이면서 특정 도메인 요구사항에 맞게 모델을 맞춤화할 수 있게 해줍니다. 이러한 더 작고 파인튜닝된 모델들은 문서 요약, 고객 지원, 예측 분석과 같은 많은 사용 사례에서 더 큰 LLM의 대규모 인프라와 리소스를 필요로 하지 않으면서 경쟁력 있는 성능을 제공할 수 있습니다.

Kubernetes는 이미 대부분의 엔터프라이즈의 전략적 런타임 컨테이너 플랫폼이기 때문에 이러한 AI 워크로드를 운영화하는 데 중요한 역할을 합니다. Kubernetes는 분산 환경 전반에서 추론 워크로드를 효율적으로 실행하는 데 필요한 확장성, 리소스 관리, 오케스트레이션을 제공합니다. 모델 훈련 및 추론 작업을 컨테이너화함으로써 조직은 AI를 기존 지속적 통합/지속적 배포(CI/CD) 파이프라인에 통합하고 대규모로 워크로드를 관리할 수 있습니다. 더욱이 KServe와 같은 도구는 모델 훈련, 배포, 모니터링 자동화를 가능하게 하여 팀이 비용 효율적이고 프로덕션 준비 상태의 방식으로 AI를 도입할 수 있게 합니다. Kubernetes와 엔터프라이즈 AI 도입 간의 이러한 시너지는 현재 대부분의 조직에서 진행 중인 플랫폼 엔지니어링 노력과 일관된 사용자 경험, 자가 서비스, 향상된 규정 준수를 제공하기 위해 잘 정렬됩니다.

## 게이트웨이 API 확장을 통한 모델 추론 제어력 향상

Kubernetes 커뮤니티는 WG Serving 워킹 그룹이 개발하고 SIG Apps가 후원하는 LLM Instance Gateway 제안과 같은 이니셔티브를 통해 추론 워크로드에 대한 지원을 적극적으로 발전시키고 있습니다. 이 맥락에서 추론 워크로드는 Kubernetes에서 이미 훈련된 모델을 실행하는 것을 의미하며, 일반적으로 GPU와 같은 특화된 하드웨어를 활용합니다. 이 제안은 LLM 서비스 호출에 맞춤화된 특화된 로드밸런싱 알고리즘, 사용자 정의 리소스 정의(CRD), 컨트롤러를 도입하여 특화된 가속기와 파인튜닝된 모델을 고려하면서 실행 중인 모델로의 요청에 대한 효율적인 라우팅 및 리소스 관리를 가능하게 합니다. 공유 컴퓨트 풀에 대한 LLM 서비스 다중화(multiplexing)를 통해 이 이니셔티브는 리소스 활용을 최적화하고 추론 성능을 개선하여 Kubernetes 환경에 AI 워크로드를 배포하는 고유한 과제들을 해결하는 것을 목표로 합니다. 이러한 노력은 Kubernetes의 현대 애플리케이션을 위한 전략적 컨테이너 런타임으로서의 역할과 일치하며, 확장성, 효율성, 운영의 용이성을 유지하면서 엔터프라이즈 인프라에 최첨단 AI 기능 통합을 더욱 간소화합니다.

## 자율적이과 행위중심의 워크플로우

AI의 미래는 LLM을 수동적인 챗봇에서 정교한 다단계 계획 및 실행이 가능한 능동적인 자율 에이전트로 변환하는 데 초점을 맞추고 있습니다. 에이전틱 AI 시스템—사용자를 대신하여 독립적으로 행동하고 결정을 내릴 수 있는 시스템—은 현재 능력을 넘어서는 상당한 진화를 나타냅니다. 사용자 프롬프트에 수동적으로 반응하는 전통적 LLM과 달리, 이러한 차세대 AI 에이전트는 일정 수준의 자율성을 가지고 목표를 동적으로 분석하고, 작업의 우선순위를 정하며, 지속적인 인간의 감시 없이 다양한 도메인 전반에 걸쳐 이들을 실행할 것입니다. 예를 들어, 보고서 생성에 관한 쿼리에 단순히 답하는 대신, AI 에이전트는 자율적으로 관련 데이터를 수집하고, 통찰력을 종합하며, 문서를 포맷팅하고, 심지어 이해관계자들을 위한 발표까지 일정을 잡을 수 있습니다.

이러한 발전은 LLM을 강화 학습, 계획 알고리즘, 외부 시스템과 통합하는 것에 달려 있습니다. 도구와 API를 활용함으로써 자율 AI 에이전트는 다른 소프트웨어, 데이터베이스, IoT(사물 인터넷) 기기와 원활하게 상호작용할 수 있으며, 비즈니스 워크플로우 자동화, 클라우드 인프라 관리, 심지어 복잡한 시뮬레이션 실행과 같은 작업을 수행할 수 있게 합니다. CrewAI 또는 LangChain과 같은 프레임워크는 이 방향의 초기 단계를 보여주며, AI 에이전트가 고수준의 목표를 실행 가능한 부작업으로 분해하고 반복적으로 실행할 수 있게 합니다. 이러한 에이전트가 더욱 정교해짐에 따라, 고객 서비스에서 로봇 공학까지 다양한 산업에 혁명을 일으킬 수 있으며, 전통적인 대화 기능을 훨씬 넘어서는 능동적이고 목표 중심의 AI 시스템의 새로운 시대를 열 것입니다.

## 다음 단계

엔터프라이즈가 AI를 운영에 계속 통합함에 따라, 특히 보안, 관찰성, 규정 준수, 확장성 영역에서 AI 워크로드를 관리하는 복잡성이 점점 더 명백해지고 있습니다. 이러한 과제를 해결하기 위해 Gloo AI Gateway(KGateway)와 Portkey AI Gateway와 같은 도구가 엔터프라이즈 AI 생태계의 필수 구성요소로 등장했습니다. AI 게이트웨이를 구현함으로써 엔터프라이즈는 AI 혁신을 가속화할 뿐만 아니라 AI 애플리케이션이 안전하게, 효율적으로, 규모에 맞춰 배포되도록 보장하는 견고한 프레임워크를 구축할 수 있습니다. 이는 조직이 AI의 변혁적 잠재력을 효과적으로 활용하면서 운영 복잡성에 대한 통제를 유지할 수 있도록 합니다.

## 저자에 관하여

**Christian Posta**(@christianposta)는 Solo.io의 VP, Global Field CTO입니다. 그는 Istio in Action(Manning)과 클라우드 네이티브 아키텍처에 관한 많은 다른 저서의 저자이며, 서비스 메쉬 및 클라우드 네이티브 생태계(Istio, Kubernetes 등)의 다양한 오픈 소스 프로젝트에서 연사, 블로거, 기여자로 활동하는 클라우드 네이티브 커뮤니티에서 잘 알려져 있습니다. Christian은 정부 및 상업 엔터프라이즈와 웹 규모의 회사에서 근무했으며, 현재 조직이 대규모의 클라우드 네이티브의 복원력 있는 분산 아키텍처를 만들고 배포하는 데 도움을 줍니다. 그는 분산 시스템 개념, 마이크로서비스, DevOps, 클라우드 네이티브 애플리케이션 설계를 통해 팀이 성공하도록 멘토링, 교육, 리더십을 즐깁니다.

**Peter Jausovec**은 Solo.io의 선임 주요 기술 마케팅 관리자이며 소프트웨어 개발, QA, 엔지니어링 리더십에 걸친 17년 이상의 경험을 보유하고 있습니다. 클라우드 네이티브 기술의 인정받는 전문가로, 그는 Kubernetes와 Istio를 전문으로 합니다. Peter는 Cloud Native(O'Reilly)라는 책을 저술했으며, 15,000명 이상의 학습자를 교육한 포괄적인 Envoy 프록시 및 Istio 교육 과정 개발을 포함한 여러 업계 선도적 이니셔티브를 개척했습니다. 그는 현재 Linux Foundation 산하의 Istio Certified Associate 시험인 첫 번째 Istio 인증 프로그램을 만들었습니다. Solo.io에서 Peter는 AI 게이트웨이 프로토타이핑을 포함하여 차세대 기술 형성에 중요한 역할을 해왔습니다.